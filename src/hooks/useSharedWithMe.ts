  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SharedWithMeAsset[]> => {
      ...
      return data.map((item: SharedAsset): SharedWithMeAsset => {
        ...
        const thumbnailMetadata = item.asset.metadata.find(
          m => m.key === 'image_variants' || m.key === 'video_variants'
        );
        if (thumbnailMetadata?.value?.thumbnails?.[0]?.path) {
          thumbnailUrl = buildAssetUrl(
            thumbnailMetadata.value.thumbnails[0].path
          );
        }
        ...
        if (
          ...
          thumbnailMetadata?.value?.original?.duration
        ) {
