...
export function ShareInfoModal({ open, onClose, asset }: ShareInfoModalProps) {
  ...
  const publicShares = asset?.shares?.filter(share => share.share_type === 'PUBLIC' && share.is_active) || []
  const restrictedShares = asset?.shares?.filter(share => share.share_type === 'RESTRICTED' && share.is_active) || []
      ...
      <div className="w-full">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 truncate" title={asset?.name}>{asset?.name}</p>
          </div>
        </div>
          ...
          {publicShares.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-gray-700">Public Share</span>
                <span className="text-xs text-gray-500">({publicShares.length} link{publicShares.length > 1 ? 's' : ''})</span>
              </div>
                    ...
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-green-800">Anyone with link</div>
          ...
          {restrictedShares.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-gray-700">Restricted Share</span>
                <span className="text-xs text-gray-500">({restrictedShares.length} user{restrictedShares.length > 1 ? 's' : ''})</span>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-blue-800 mb-2">
                        Shared with {restrictedShares.length} user{restrictedShares.length > 1 ? 's' : ''}
                      </div>
                      <div className="space-y-1">
                        {restrictedShares.map((share) => (
                          <div key={share.id} className="text-xs text-blue-600">
                            • {share.user ? share.user.full_name : 'Unknown User'} ({share.user?.email})
                          </div>
                        ))}
                      </div>
                      <div className="text-xs text-blue-500 mt-2">
                        Created {new Date(restrictedShares[0].created_at).toLocaleString()}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
