...
const AssetPreviewModal = lazy(() =>
  import('../components/AssetPreviewModal').then(m => ({
    default: m.AssetPreviewModal,
  }))
);
const ShareModal = lazy(() =>
  import('../components/ShareModal').then(m => ({ default: m.ShareModal }))
);
const ShareInfoModal = lazy(() =>
  import('../components/ShareInfoModal').then(m => ({
    default: m.ShareInfoModal,
  }))
);
...
export default function AssetsPage() {
  ...
  const [shareInfoModal, setShareInfoModal] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const getShareStatus = (asset: any) => {
    ...
    const hasPublic = activeShares.some(
      (share: any) => share.share_type === 'PUBLIC'
    );
    const hasRestricted = activeShares.some(
      (share: any) => share.share_type === 'RESTRICTED'
    );

    if (hasPublic)
      return {
        type: 'PUBLIC',
        count: activeShares.filter((s: any) => s.share_type === 'PUBLIC')
          .length,
      };
    if (hasRestricted)
      return {
        type: 'RESTRICTED',
        count: activeShares.filter((s: any) => s.share_type === 'RESTRICTED')
          .length,
      };
  ...
  const handleShare = (
    ...
    const hasActiveShares =
      asset?.shares?.some(share => share.is_active) || false;
    ...
    <div>
                ...
                <div className='divide-y divide-gray-100'>
                  ...
                  {data?.map(a => {
                    ...
                    return (
                      <div
                        key={a.id}
                        className='grid grid-cols-[auto_1fr] md:grid-cols-[auto_1fr_100px_140px_140px_80px_60px] items-center gap-3 px-4 py-3 hover:bg-gray-50 cursor-pointer'
                        onClick={() =>
                          setSelected({
                            id: a.id,
                            name: a.name,
                      ...
                      >
                        ...
                        <div className='min-w-0'>
                          ...
                          <div className='text-xs text-gray-500 truncate'>
                            {a.uploader
                              ? a.uploader.id === userId
                                ? 'Added by You'
                                : `Added by ${a.uploader.fullName}`
                              : null}
                          </div>
                        </div>
                            ...
                            <div className='flex items-center gap-1 text-xs'>
                              {shareStatus.type === 'PUBLIC' ? (
                                ...
                                <div
                                  ...
                                  title={`Restricted share (${shareStatus.count} user${shareStatus.count > 1 ? 's' : ''})`}
                                >
              ...
              {data?.map(a => {
                ...
                return (
                  <Card
                    ...
                    onClick={() =>
                      setSelected({ id: a.id, name: a.name, mime: a.mimeType })
                    }
                  >
                    <div className='relative'>
                      {a.thumbnailUrl ? (
                        <img
                          src={a.thumbnailUrl}
                          alt={a.name}
                          className={`${thumbClass} w-full object-cover`}
                          loading='lazy'
                        />
                      ) : (
                        <div
                          className={`flex ${thumbClass} items-center justify-center bg-gray-100 text-gray-400`}
                        >
                          No preview
                        </div>
                      )}
                    ...
                    <CardContent>
                      ...
                      {a.uploader ? (
                        <div className='mt-1 text-xs text-gray-500'>
                          ...
                          {a.uploader.id === userId
                            ? 'You'
                            : `${a.uploader.fullName} (${a.uploader.role})`}
                        </div>
                      ) : null}
                    </CardContent>
                  </Card>
      ...
      <Suspense fallback={null}>
        ...
        <ShareInfoModal
          ...
          asset={
            shareInfoModal
              ? data?.find(a => a.id === shareInfoModal.id) || null
              : null
          }
        />
      </Suspense>
