...
export default function UsersPage() {
  ...
  const setGeneratedUrlFor = (id: string, url: string) => setUrlById((m) => ({ ...m, [id]: url }))
  const dismissGeneratedUrlFor = (id: string) => setUrlById((m) => { const { [id]: _, ...rest } = m; return rest })
  ...
  const { data: invites, isLoading, refetch } = useQuery({ queryKey: ['invitations'], queryFn: fetchInvitations })
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="mb-3 text-sm font-medium text-gray-800">Invite a new user</div>
      ...
      <div className="mt-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="mb-3 text-sm font-medium text-gray-800">Invitations</div>
        {/* Desktop table */}
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-2 text-left font-semibold text-gray-600">Invited By</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                  ...
                  <>
                    <tr key={inv.id}>
                      ...
                      <td className="px-3 py-2">
                        <span className={`rounded-md px-2 py-0.5 text-xs ${inv.status === 'PENDING' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : inv.status === 'JOINED' || inv.status === 'ACCEPTED' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-gray-50 text-gray-700 border border-gray-200'}`}>{inv.status}</span>
                      </td>
                        ...
                        <td colSpan={5} className="px-3 py-2">
                          <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs">
                            <div className="mb-1 flex items-center justify-between">
                              <div className="font-medium text-blue-800">Invitation URL</div>
                              <button onClick={() => dismissGeneratedUrlFor(inv.id)} className="rounded-md px-2 py-1 text-blue-700 hover:bg-blue-100">Dismiss</button>
                            </div>
                            <div className="flex items-center gap-2">
                              <code className="flex-1 break-all text-blue-900">{urlById[inv.id]}</code>
                              <button onClick={() => navigator.clipboard?.writeText(urlById[inv.id])} className="shrink-0 rounded-md border border-blue-200 bg-white px-2 py-1 text-xs text-blue-700 hover:bg-blue-100">Copy</button>
                            </div>
                          </div>
                        </td>
                      </tr>
                ...
                <tr><td className="px-3 py-3 text-gray-500" colSpan={5}>No invitations</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="md:hidden">
          {isLoading ? (
            ...
            <div className="space-y-2">
              {invites.map((inv) => (
                <details key={inv.id} className="rounded-xl border border-gray-200 bg-white p-3">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-gray-800">{inv.invitedBy.full_name}</div>
                    </div>
